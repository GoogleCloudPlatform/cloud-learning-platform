...
'''
@behave.given("Required User already exists in the database and correct uuid will be used to find user")
...
@behave.when("A GET api call is made to the User Management Service with correct uuid")
...
'''
@behave.given("Required User does not exists in the database and incorrect uuid will be used to find the user")
def step_impl_1(context):
    context.uuid = "some_random_uuid"
    context.get_user_url = f"{UM_API_URL}/user/{context.uuid}"

@behave.when("A GET api call is made to the User Management Service with incorrect uuid")
def step_impl_2(context):
...
def step_impl_3(context):
    ...
    assert context.get_user_res.json()["message"] == f"User with user_id {context.uuid} not found"


'''
Scenario: User information is to be searched with user email
'''
@behave.given("Required User already exists in the database and correct email address will be used for query")
def step_impl_1(context):
...
@behave.when("A GET api call is made to the User Management Service with correct email")
def step_impl_2(context):
...
@behave.when("A GET api call is made to the User Management Service to fetch all")
def step_impl_2(context):
...
def step_impl_2(context):
    ...
    context.get_user_res = get_method(url= get_user_url,
        query_params={"search_query": context.firstname})

@behave.then("User management will successfully return users for the given filter")
def step_impl_3(context):
    assert context.get_user_res.status_code == 200
    assert context.get_user_res.json()["data"][0]["first_name"] == context.firstname