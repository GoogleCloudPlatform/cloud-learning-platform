...
assessment_service_url = get_baseurl("assessment-service")
...
if not lrs_url:
  raise ResourceNotFoundException(
      "Unable to locate the service URL for learning-record-service")
else:
...
if not los_url:
  raise ResourceNotFoundException(
      "Unable to locate the service URL for learning-object-service")
else:
  API_URL_LEARNING_OBJECT_SERVICE = f"{los_url}/learning-object-service/api/v1"

if not slp_url:
  raise ResourceNotFoundException(
      "Unable to locate the service URL for student-learner-profile")
else:
  API_URL_LEARNER_PROFILE_SERVICE = f"{slp_url}/learner-profile-service/api/v1"

if not assessment_service_url:
  raise ResourceNotFoundException(
      "Unable to locate the service URL for assessment-service")
else:
  API_URL_ASSESSMENT_SERVICE = f"{assessment_service_url}/assessment-service/api/v1"
