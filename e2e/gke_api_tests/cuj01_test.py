...
def test_get_course_list(get_token):
  res = requests.get(f"{API_URL}/sections/get_courses/",headers=get_token)
  assert res.status_code == 200


def test_copy_course(get_token):
  ...
  res = requests.post(url=API_URL + "/sections/copy_course/",
                    json=course_details,headers=get_token)
  assert res.status_code == 200
