...
def prepare_json():
    ...
    with open(f"student_json_" + csv_file_name, "w") as file:
        file.write(str(STUDENT_JSON))
    print("Length of Student JSON is ----------------", len(STUDENT_JSON))
    print("Length of Studnet Array is -----------------", len(STUDENT_ARRAY))
...
def push_grade(email, classroom_id, coursework_id, draft_grade,
               assigned_grade):
        ...
        for data in data_list:
            runner = loop.run_in_executor(
                ...
                *(data.get("email"), data.get("classroom_id"),
                  data.get("coursework_id"), data.get("score"),
                  data.get("score")))
            tasks.append(runner)
