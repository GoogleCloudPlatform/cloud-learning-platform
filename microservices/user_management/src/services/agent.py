...
# pylint: disable = broad-exception-raised
def create_agent(headers: dict = None, agent_dict: dict = None):
  """Post request to learner record service to create agent"""
  Logger.info(agent_dict)
...
def delete_agent(headers: dict = None, agent_id: str = None):
...
def get_agent(headers: dict = None, user_id: str = None):
  ...
  agent_id = response.json().get("data")["records"][0].get("uuid")
  return agent_id
