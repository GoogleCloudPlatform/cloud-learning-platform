...
def apply_filters(doc_ref, filters):
  ...
  for filter_ in filters:
    doc_ref = doc_ref.where("{}".format(filter_), "==", filters[filter_])
  return doc_ref
...
def insert_document(collection_name, data):
  ...
  doc_id = get_doc_id()
  doc_insert_status = db.collection("{}".format(collection_name)).document(
    "{}".format(doc_id)).set(data)
...
def check_document(collection_name, field_path, value):
  """
    ...
    """
  db = get_firestore_instance()
  docs_ref = db.collection("{}".format(collection_name)) \
    .where("{}".format(field_path), "==", "{}".format(value)).limit(1)
  docs = docs_ref.get()
  doc_list = list(docs)
  if len(doc_list) > 0:
...
def get_documents(collection_name):
  """
    ...
    """
  db = get_firestore_instance()
  doc_ref = db.collection("{}".format(collection_name))
  docs = doc_ref.get()
  return docs
...
def get_document_by_id(collection_name, doc_id):
  ...
  db = get_firestore_instance()
  doc = db.collection("{}".format(collection_name)).document(
    "{}".format(doc_id)).get()
  return doc
...
def update_document(collection_name, doc_id, data):
  """
    ...
    """
  db = get_firestore_instance()
  doc_ref = db.collection("{}".format(collection_name)).document(
    "{}".format(doc_id))
  doc_ref.update(data)
  return doc_ref.id
...
def get_leaf_document(collection_name, filters=None):
  """
    ...
    """
  db = get_firestore_instance()
  doc_ref = db.collection_group("{}".format(collection_name))
...
def check_session_exists(collection_name, filters=None):
  ...
  db = get_firestore_instance()
  doc_ref = db.collection("{}".format(collection_name))
  docs = doc_ref.where("user_id", "==", filters["user_id"]). \
    where("is_active", "==", filters["is_active"]). \
    where("course_ref", "==", filters["course_ref"]). \
    where("context_ref", "==", filters["context_ref"]). \
    where("competency_ref", "==", filters["competency_ref"]).stream()
  res = list(docs)
