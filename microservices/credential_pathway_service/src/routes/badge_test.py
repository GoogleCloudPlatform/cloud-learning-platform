"""
  Unit tests for badge endpoints
"""
import os
import json
...
from routes.badge import router
...
BADGE_TESTDATA_FILENAME = os.path.join(TESTING_FOLDER_PATH, "badge.json")

client_with_emulator = TestClient(app)

# assigning url
api_url = f"{API_URL}/badge"

os.environ["FIRESTORE_EMULATOR_HOST"] = "localhost:8080"
os.environ["GOOGLE_CLOUD_PROJECT"] = "fake-project"


def test_get_badge(clean_firestore):
  badge_dict = BASIC_BADGE_EXAMPLE
  badge = BadgeClass.from_dict(badge_dict)
  badge.uuid = ""
  badge.save()
  badge.uuid = badge.id
  badge.update()
  badge_dict["uuid"] = badge.id

  url = f"{api_url}/{badge.uuid}"
  ...
  assert json_response.get("data") == badge_dict, "Response received"


def test_get_badge_negative(clean_firestore):
  uuid = "ASS345Dl3Ayg0PWudzhI"
  url = f"{api_url}/{uuid}"
  data = {
      "success": False,
      "message": f"BadgeClass with uuid {uuid} not found",
      "data": None
  }
...
def test_post_badge(clean_firestore):
  input_badge = BASIC_BADGE_EXAMPLE
  url = api_url
  post_resp = client_with_emulator.post(url, json=input_badge)
  ...
  loaded_badge = BadgeClass.find_by_uuid(uuid)
  loaded_badge_dict = loaded_badge.to_dict()

  # popping id and key for equivalency test
  loaded_badge_dict.pop("id")
  loaded_badge_dict.pop("key")
  loaded_badge_dict.pop("created_time")
  loaded_badge_dict.pop("last_modified_by")
  loaded_badge_dict.pop("last_modified_time")


def test_update_badge(clean_firestore):
  badge_dict = BASIC_BADGE_EXAMPLE
  badge = BadgeClass.from_dict(badge_dict)
  badge.uuid = ""
  badge.save()
  ...
  url = f"{api_url}/{badge_uuid}"
  updated_data = {
      "entity_type": "badge",
      "entity_id": "12345",
      "open_badge_id": "12",
  }
  updated_data["achievement_type"] = "Achievement"
  ...
  assert json_response_update_req.get(
      "message"
  ) == "Successfully update the badge", "Expected response not same"
  assert json_response_update_req.get("data").get(
      "achievement_type") == "Achievement", "Expected response not same"


def test_update_badge_negative(clean_firestore):
  badge_dict = {
      "entity_type": "badge",
      "entity_id": "12345",
      "open_badge_id": "12",
  }
  badge_uuid = "ASS345Dl3Ayg0PWudzhI"
  url = f"{api_url}/{badge_uuid}"
  response = {
      "success": False,
      "message": "BadgeClass with uuid ASS345Dl3Ayg0PWudzhI not found",
      "data": None
  }
  resp = client_with_emulator.put(url, json=badge_dict)
  json_response = resp.json()
...
def test_delete_badge(clean_firestore):
  badge_dict = BASIC_BADGE_EXAMPLE
  badge = BadgeClass.from_dict(badge_dict)
  badge.uuid = ""
  badge.save()
...
def test_import_badge(clean_firestore):
  url = f"{api_url}/import/json"
  with open(BADGE_TESTDATA_FILENAME, encoding="UTF-8") as badge_json_file:
    resp = client_with_emulator.post(url, files={"json_file": badge_json_file})
