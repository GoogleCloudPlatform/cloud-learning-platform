...
def test_get_course_template_list(client_with_emulator):
    ...
    assert len(response_json["course_template_list"]) == len(COURSE_TEMPLATE_LIST_TEST_DATA), "Return data list len doesn't match."
...
def test_create_course_template(client_with_emulator):
    ...
    assert response_json["course_template"]["name"] == INSERT_COURSE_TEMPLATE_EXAMPLE["name"], "Check the response course template name"
