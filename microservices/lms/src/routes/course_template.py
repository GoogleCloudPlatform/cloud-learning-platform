...
def get_instructional_designer(course_template_id: str,
                               instructional_designer: str, request: Request):
  """_summary_
  ...
  try:
    ...
    if result is None:
      raise ResourceNotFoundException(
          "Instructional Designer not found in this" +
          f" Course Template {course_template_id}")
    data = course_template_enrollment_instructional_designer_model(result)
    return {"data": data}
  except HttpError as hte:
    Logger.error(hte)
    raise ClassroomHttpException(status_code=hte.resp.status,
...
@router.get("/{course_template_id}/instructional_designers",
...
def list_template_instructional_designer(course_template_id: str):
  ...
  try:
    course_template = CourseTemplate.find_by_id(course_template_id)
    instructional_designers = CourseTemplateEnrollmentMapping\
      .fetch_all_by_course_template(
        course_template.key)
    if instructional_designers is None:
      raise ResourceNotFoundException(
          ...
          f" Course Template {course_template_id}")
    data = [
        course_template_enrollment_instructional_designer_model(i)
        for i in instructional_designers
    ]
    return {"data": data}
  except HttpError as hte:
    Logger.error(hte)
