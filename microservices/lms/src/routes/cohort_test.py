...
def test_get_overall_percentage(client_with_emulator,create_fake_data):
  url = BASE_URL+f"/cohorts/{create_fake_data['cohort']}/get_overall_percentage/student@gmail.com"
  course_work_data = [{
  ...
  with mock.patch("routes.cohort.student_service.get_user_id",return_value="student@gmail.com"):
    with mock.patch("routes.cohort.classroom_crud.get_coursework",return_value=course_work_data):
      with mock.patch("routes.cohort.classroom_crud.get_submitted_course_work_list",return_value=submitted_course_work_data):
        resp = client_with_emulator.get(url)
  assert resp.status_code == 200
