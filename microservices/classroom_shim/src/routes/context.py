...
@router.get("/contexts/{context_id}", response_model=ContextResponseModel)
def get_context_info_by_id(context_id: str):
...
@router.get(
    "/contexts/{context_id}/members", response_model=ContextResponseModel)
  ...
  try:
    ...
    if context_type == "course_template":
      ...
      if get_ids_res.status_code == 200:
        ...
        Logger.error(
            f"Error 1130 response: Status code: {get_ids_res.status_code}; Response: {get_ids_res.text}"
        )
